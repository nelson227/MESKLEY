...
export default function PhotoGallery({ photos, videos = [], title }: PhotoGalleryProps) {
  ...
  if (media.length === 0) {
    media.push({ url: "https://images.unsplash.com/photo-1560448204-e02f11c3d0e2?w=1200&q=80", type: "image" });
  }
      ...
      {media.length > 1 && (
        <div className="flex gap-2 mt-3 overflow-x-auto pb-2">
          {media.map((item, i) => (
            ...
            >
              {item.type === "image" ? (
                <img src={item.url} alt={`Miniature ${i + 1}`} className="w-full h-full object-cover" />
              ) : (
      ...
      {isLightbox && (
        <div className="fixed inset-0 z-[70] bg-black/95 flex items-center justify-center" onClick={() => setIsLightbox(false)}>
          ...
          <div className="max-w-[90vw] max-h-[90vh]" onClick={(e) => e.stopPropagation()}>
            {current.type === "image" ? (
              <img src={current.url} alt={`${title} - Photo ${currentIndex + 1}`} className="max-w-full max-h-[85vh] object-contain" />
            ) : (
              <video src={current.url} className="max-w-full max-h-[85vh]" controls autoPlay />
            )}
          </div>
          {media.length > 1 && (
            <>
